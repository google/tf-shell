#!/bin/bash -eu
#
# This file originally came from from tensorflow's example custom op repository
# available here:
# https://github.com/tensorflow/custom-op/blob/3799c2d6f21b8ff69e70b1dbcf2a3b5f631d6e56/configure.sh
...
write_to_bazelrc "build --enable_bzlmod"
write_to_bazelrc "build --cxxopt='-std=c++17'"
write_to_bazelrc "build --cxxopt='-DPYBIND11_ABSEIL_STATUS_MODULE_PATH=pybind11_abseil.pybind11_abseil.status'"

# Known issue with transitive dependencies https://github.com/bazelbuild/bazel/issues/15359
write_to_bazelrc "build --spawn_strategy=sandboxed"

# allows compiling with -c dbg --config=asan
write_to_bazelrc "build:asan --strip=never"
write_to_bazelrc "build:asan --copt -fsanitize=address"
write_to_bazelrc "build:asan --copt -DADDRESS_SANITIZER"
write_to_bazelrc "build:asan --copt -O1"
write_to_bazelrc "build:asan --copt -g"
write_to_bazelrc "build:asan --copt -fno-omit-frame-pointer"
write_to_bazelrc "build:asan --linkopt -fsanitize=address"

# Module import path changes depending on if imported in bazel tests or in
# the released and pip installed wheel.
write_to_bazelrc "build:release --cxxopt='-DPYBIND11_ABSEIL_STATUS_MODULE_PATH=pybind11_abseil.status'"